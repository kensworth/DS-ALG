...
public class DirectorySize {
	...
	public static void main(String[] args) throws IOException{
		...
		if(args[0] == null) {
			throw new IOException("File must be run with 1-2 arguments");
		}
		...
		for (int i = 0; i < numOfFiles; i++) {
			...
			System.out.println(listOfFiles.get(listOfFiles.size() - i - 1).size() + " " + listOfFiles.get(listOfFiles.size() - i - 1).getPath());
		}
	}
	...
	public static long getSize (File file) throws IOException   {
		...
		for(int j = 0; j < listOfFiles.size(); j++) {
			System.out.println(listOfFiles.get(j).getPath() + " " + listOfFiles.get(j).size());
		}

		return size;	
	}

	public static long exploreDir(File file) {
		if(file.isDirectory()) {
			size += file.length();

			FileOnDisk fileOnDisk = new FileOnDisk(file.toString(), file.length());
			listOfFiles.add(fileOnDisk);

			File[] fileArray = file.listFiles();
			for(int i = 0; i < fileArray.length; i++) {
				File recursiveFile = new File(fileArray[i].toString()); 
				exploreDir(fileArray[i]);
			}
		}
		else {
			FileOnDisk fileOnDisk = new FileOnDisk(file.toString(), file.length());
